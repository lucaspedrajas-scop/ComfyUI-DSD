...
class FluxConditionalPipeline(DiffusionPipeline, SD3LoraLoaderMixin):
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            # Debug the image shape before splitting
            print(f"Image shape before splitting: {image.shape}")
            
            if cut_output:
                # Store the reference image (left side)
                reference_image = image[...,:width//2].clone()  # Use clone to ensure we have a separate copy
                # Store the output image (right side)
                image = image[..., width//2:]
                
                # Debug the shapes after splitting
                print(f"Reference image shape after splitting: {reference_image.shape}")
                print(f"Output image shape after splitting: {image.shape}")
                
            # Post-process the images
            image = self.image_processor.postprocess(image, output_type=output_type)
            if reference_image is not None:
                reference_image = self.image_processor.postprocess(reference_image, output_type=output_type)
                print(f"Reference image type after postprocessing: {type(reference_image)}")
                if isinstance(reference_image, list):
                    print(f"Reference image list length: {len(reference_image)}")
                    if len(reference_image) > 0:
                        print(f"First reference image type: {type(reference_image[0])}")
                        print(f"First reference image size: {reference_image[0].size if hasattr(reference_image[0], 'size') else 'unknown'}")
        ...
        return FluxPipelineOutput(images=(image, reference_image))
...
class FluxPipelineOutput(BaseOutput):
    ...
    Args:
        images (`List[PIL.Image.Image]` or `np.ndarray` or tuple)
            List of denoised PIL images of length `batch_size` or numpy array of shape `(batch_size, height, width,
            num_channels)` or tuple of (output_image, reference_image). PIL images or numpy array present the denoised 
            images of the diffusion pipeline.
    """

    images: Union[List[PIL.Image.Image], np.ndarray, tuple]
    
    def __post_init__(self):
        # Ensure images is always a tuple of (output_image, reference_image)
